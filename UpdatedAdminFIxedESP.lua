--[[
    @Author: Zuka Tech
    @Date: 11/6/2025 (Refactored for Robustness & Auto-Completion)
    @Description: A modular, client-sided chat command system for Zuka in Roblox.
                 ...
                 command bar with real-time auto-completion.
    @Refinement:
        - Overhauled the ESP module to be event-driven. It now correctly handles players joining,
          leaving, and respawning without needing to be toggled.
        - Improved the clarity of the ;cmds UI by reformatting how aliases are displayed.
]]
...
function Modules.ESP:_setupPlayer(player)
    if player == LocalPlayer then return end
    self:_cleanupPlayer(player) -- Clean up any old data before setup

    local playerData = { Billboard = nil, Highlight = nil, Connections = {} }
    self.State.TrackedPlayers[player] = playerData

    local function onCharacter(character)
        -- Clean up previous visuals on respawn
        if playerData.Highlight and playerData.Highlight.Parent then playerData.Highlight:Destroy() end
        if playerData.Billboard and playerData.Billboard.Parent then playerData.Billboard:Destroy() end
        ...
        -- Connect to the Died event for instant cleanup
        table.insert(playerData.Connections, humanoid.Died:Connect(function()
            if playerData.Highlight then playerData.Highlight:Destroy() end
            if playerData.Billboard then playerData.Billboard:Destroy() end
            playerData.Highlight = nil
            playerData.Billboard = nil
        end))
    end
    
    table.insert(playerData.Connections, player.CharacterAdded:Connect(onCharacter))
    if player.Character then onCharacter(player.Character) end
end
function Modules.ESP:Toggle()
    self.State.IsActive = not self.State.IsActive
    if self.State.IsActive then
        local playerAddedConn = Players.PlayerAdded:Connect(function(p) self:_setupPlayer(p) end)
        local playerRemovingConn = Players.PlayerRemoving:Connect(function(p) self:_cleanupPlayer(p) end)
        self.State.Connections = {playerAddedConn, playerRemovingConn}
        for _, player in ipairs(Players:GetPlayers()) do
            self:_setupPlayer(player)
        end
        DoNotif("ESP Enabled", 3)
    else
...
DoNotif("Zuka Command Handler v19 (Refined) | Prefix: '" .. Prefix .. "' | ;cmds for help", 6)